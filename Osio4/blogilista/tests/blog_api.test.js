...
test('a specific blog is in the returned blogs', async () => {
    ...
    expect(contents).toContainEqual('First class tests')
})
...
test('a note can be added', async () => {
    ...
    expect(contents).toContainEqual('CV')
...
test('blog without title is not added', async () => {
    ...
    expect(response.body.length).toBe(responseAll.body.length)
})
...
test('a note with no likes value gets likes value 0', async () => {
    const newBlog = {
        title: 'CV',
        author: 'Josefin Reuter',
        url: 'www.josefinreuter.fi',
    ...
    expect(responseAll.body.length).toBe(blogs.length + 1)
